...
class _AddProductsState extends State<AddProducts> {
  ...
  Future<void> _pickImage() async {
   ...
   if (pickedFile != null) {_imageNotifier.value = File(pickedFile.path);}}
  ...
  Future<void> _addProduct() async {
    if (!_validateInputs() || _formKey.currentState?.validate() != true) {log("Validation failed.");
      CustomSnackBarCustomisation.show(context: context,message: 'Please fill in all fields. !',
       messageColor: blue,icon: Icons.info_outline_rounded,iconColor: blue);
      return;}
    ...
    if (success) {
      ...
      CustomSnackBarCustomisation.show(context: context,message: "Failed to add product.!",
          messageColor: redColor,icon: Icons.sms_failed_outlined,iconColor: redColor);
    }
  }
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              ...
              child: Text( "OK",style: TextStyle(color: black),),
            ),
          ],
        );
      },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: MyAppBarTwo(titleText: 'Add Product', bgColor: inside,),
            ...
            child: Column(
              children: [
                        ...
                        child: ValueListenableBuilder<File?>(
                          ...
                          builder: (context, imageFile, child) {
                            if (imageFile != null) {
                              ...
                              return Lottie.asset('assets/category/addimage.json', fit: BoxFit.contain,
                              );}},), ),),),),
                SizedBox(height: screenHeight * 0.07),
                const FieldText(text: 'Item Name', icon: Icons.brightness_auto,),
                FieldDecoration(controller: _itemNameController,height: screenHeight * 0.06,
                hintText: 'Name of the item',
                validate: InputValidator.validate,),
                SizedBox(height: screenHeight * 0.01),
                const FieldText( text: 'Description',icon: Icons.description_outlined,),
                FieldDecoration(controller: _descriptionController,height: screenHeight * 0.15, 
                hintText: 'Description of the item',
                validate: InputValidator.validate,),
                SizedBox(height: screenHeight * 0.01),
                const FieldText(text: 'Purchase Price',icon: Icons.rocket_launch,),
                DigitField(
                ...
                const Padding( padding: EdgeInsets.symmetric(vertical: 4.0),
                  child: OrCall(text: 'Purchase Information', ),
                ), const FieldText(text: 'Select Category',icon: Icons.move_down_sharp, ),
                Padding(
                  padding: const EdgeInsets.symmetric(vertical: 15.0),
                  child: DropDown( height: screenHeight * 0.065,
                    hintText: 'Category', selectedCategoryNotifier: _selectedCategoryNotifier,),
                ),
                SizedBox(height: screenHeight * 0.01),
                const FieldText( text: 'Maximum Retail Price', icon: Icons.calculate_outlined),
                DigitField(controller: _mrpController,height: screenHeight * 0.06,
                  hintText: 'MRP', validate: DigitInputValidator.validate,
                ), SizedBox(height: screenHeight * 0.01),
                Row(
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: () {
                        setState(() {
                          if (_quantity > 1) {
                            _quantity--; _quantityColor = redColor;
                          }});_updateTotalPrice();
                      },
                      ...
                      child: Text(
                        '$_quantity', style: TextStyle(
                          fontSize: 19,color: _quantityColor,
                        ),),
                    ),
                    ElevatedButton(
                      onPressed: () {
                ...
                ValueListenableBuilder<double>(
                  ...
                  builder: (context, totalPrice, child) {
                    return CheckOut(color: const Color.fromARGB(255, 10, 73, 110),
                     height: screenHeight * 0.06,
